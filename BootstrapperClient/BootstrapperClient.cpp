...
static const TCHAR* BootstrapperMutexName   = _T("www.pekora.org/bootstrapper");
static const TCHAR* StartRobloxAppMutex     = _T("www.pekora.org/startRobloxApp");
...
bool BootstrapperClient::NeedPreDeployRun()
{
	...
	CRegKey key;
	if (SUCCEEDED(key.Open(HKEY_CURRENT_USER, _T("Software\\Pekora2 Corporation\\Pekora2"), KEY_READ)))
	{
		TCHAR buf[MAX_PATH];
...
void BootstrapperClient::RunPreDeploy()
{
	...
	try
	{
		...
		CRegKey key;
		if (SUCCEEDED(key.Create(HKEY_CURRENT_USER, _T("Software\\Pekora2 Corporation\\Pekora2"))))
		{
...
void BootstrapperClient::StartRobloxApp(bool fromInstall)
{
	...
	{
		LOG_ENTRY("Another process is starting Pekora2. Abandoning startRobloxApp");
		return;
	}
	...
	message("Starting Pekora2...");

	LOG_ENTRY("Creating event");
	CEvent robloxStartedEvent(NULL, TRUE, FALSE, _T("www.pekora.org/robloxStartedEvent"));
...
void BootstrapperClient::deployRobloxProxy(bool commitData)
{
...

	{
		...
		CreateKey(allowedDomainsKey->m_hKey, _T("pekora.org"));
...
void BootstrapperClient::registerFirefoxPlugin(const TCHAR* id, bool is64Bits)
{
	...
	key->SetStringValue(_T("Description"), _T("Pekora2 Launcher"));
	key->SetStringValue(_T("Vendor"), _T("Pekora2"));
