	...
	int WinMain(HINSTANCE hInstance, int nShowCmd, Bootstrapper*(*newBootstrapper)(HINSTANCE)) throw()
	{
		...
		int result = 0;
		{
			...
			catch (std::exception& e)
			{
				...
				{
					CString message;
					message.Format(_T("An error occured and Pekora2 cannot continue.\n\n%S"), e.what());
...
boost::shared_ptr<Bootstrapper> Bootstrapper::Create(HINSTANCE hInstance, Bootstrapper*(*newBootstrapper)(HINSTANCE))
{
	...
	if (!result->windowed)
	{
		if (!result->noRun)
		{
			...
			{
				LLOG_ENTRY(result->logger, "No bg update, Pekora2 App is running");
				return result;
			}
...
void Bootstrapper::RegisterUninstall(const TCHAR *productName)
{
	...
	throwHRESULT (keyProductCode->SetStringValue(_T("Publisher"), _T("Pekora2 Corporation")), "Failed to set Publisher key");
	throwHRESULT (keyProductCode->SetStringValue(_T("URLInfoAbout"), _T("http://pekora.org")), "Failed to set URLInfoAbout key");
...
bool Bootstrapper::checkBootstrapperVersion()
{
	...
	if (moduleVersionNumber != bootstrapperVersion)
	{
		...
		if(moduleVersionNumber != bootstrapperVersion){
			...
			try
			{
				message("Getting the latest Pekora2...");
...
void Bootstrapper::checkOSPrerequisit()
{
	...
	if (windowed)
		dialog->DisplayError("Pekora2 requires Microsoft Windows XP SP1 or greater", NULL);
...
void Bootstrapper::checkCPUPrerequisit()
{
    if (!Bootstrapper::hasSse2())
	{
	    ...
	    if (windowed)
		    dialog->DisplayError("Pekora2 requires SSE2 support", NULL);
...
void Bootstrapper::checkIEPrerequisit()
{
	if (!isIEUpToDate())
	{
		...
		if (windowed)
			dialog->DisplayError("Pekora2 requires Microsoft Internet Explorer 6.0 or greater", NULL);
...
void Bootstrapper::checkDiskSpace()
{
	...
	{
		dialog->DisplayError("There is not enough room on your disk to install Pekora2. Please free up some space and try again.", NULL);
...
void Bootstrapper::run()
{
	...
	try
	{
		...
		if (!checkBootstrapperVersion())
		{
			...
			if (windowed && isLatestProcess())
			{
				CString message = _T("Cannot connect to the Pekora2.\n\nIs antivirus software preventing Pekora2 from accessing the Internet?");
				if (!robloxAppArgs.empty())
				{
					message += _T("\n\nIf you continue Pekora2 may not work properly.");
					if (dialog->MessageBox(message, _T("Error"), MB_OKCANCEL | MB_ICONEXCLAMATION) == IDOK)
					{
						installVersion = queryInstalledVersion();
						StartRobloxApp(false);
		...
		if (isFailIfNotUpToDate)
		{
			...
			LOG_ENTRY("Pekora2 is up to date, returning success");
			goto done;
		}
		...
		if (queryInstalledVersion() == installVersion)
		{
			...
			else
			{
				...
				std::wstring pathCheckingFor;
#if VER_2017
				pathCheckingFor = programDirectory() + L"2017L\\" + GetRobloxAppFileName().c_str();
...
#else
				// No default client available if 2017 is disabled
				robloxAppFile = INVALID_HANDLE_VALUE;
				pathCheckingFor = L"<2017 client disabled>";
#endif
	...
	{
		exitCode = 1;
		LOG_ENTRY("Pekora2 is not up to date, returning failure");
	}
	catch (silent_exception&)
	{
...
void Bootstrapper::install()
{
	...
	try
	{
		if (IsWinXP())
		{
			...
			{
				if (windowed)
					dialog->DisplayError("On this machine you must install Pekora2 using an Administrator account", NULL);
...
void Bootstrapper::setLatestProcess()
{
	...
	const std::string latestProcessName = format_string("www.pekora.org/%s/%s/latestProcess", installHost.c_str(), Type().c_str());
